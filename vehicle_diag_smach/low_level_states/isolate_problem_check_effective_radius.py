...
class IsolateProblemCheckEffectiveRadius(smach.State):
    ...
    def gen_causal_graph_visualizations(self, anomalous_paths: dict, complete_graphs: dict,
        ...
        for key in complete_graphs.keys():
            ...
            plt.title("Causal Graph (Network of Effective Connections) for $\mathregular{" + key + "}$",
                      fontsize=40, fontweight='bold')

            from_relations = ["$\mathregular{" + k + "}$" for k in complete_graphs[key].keys() for _ in range(len(complete_graphs[key][k]))]
            to_relations = [complete_graphs[key][k] for k in complete_graphs[key].keys()]
            to_relations = ["$\mathregular{" + item + "}$" for lst in to_relations for item in lst]
            ...
            for i in range(len(to_relations)):
                if key in anomalous_paths.keys():
                    for j in range(len(anomalous_paths[key]) - 1):
                        # causal link check
                        if anomalous_paths[key][j] == from_relations[i].replace("$\mathregular{", "").replace("}$", "") \
                                and anomalous_paths[key][j + 1] == to_relations[i].replace("$\mathregular{", "").replace("}$", ""):
            ...
            for i in range(len(from_relations)):
                ...
                if (from_relations[i].replace("$\mathregular{", "").replace("}$", "") not in explicitly_considered_links.keys()
                        or to_relations[i].replace("$\mathregular{", "").replace("}$", "") not in \
                        explicitly_considered_links[from_relations[i].replace("$\mathregular{", "").replace("}$", "")]):
                    colors[i] = 'black'
            ...
            pos = nx.spring_layout(g, scale=0.2, seed=5)
            nx.draw(g, pos=pos, with_labels=True, node_size=10000, font_size=45, alpha=0.75, arrows=True,
